-- Script SQL ALTERNATIVO para criar sistema de gamificação
...
-- Adicionar coluna type se não existir (para tabelas já criadas)
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'missions' AND column_name = 'type' AND table_schema = 'public') THEN
        ALTER TABLE public.missions ADD COLUMN type VARCHAR(100) NOT NULL DEFAULT 'general';
    END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_missions_level ON public.missions(required_level);
...
CREATE INDEX IF NOT EXISTS idx_user_mission_progress_mission ON public.user_mission_progress(mission_id);
CREATE INDEX IF NOT EXISTS idx_user_mission_progress_completed ON public.user_mission_progress(is_completed);
...
CREATE INDEX IF NOT EXISTS idx_gamification_history_date ON public.gamification_history(created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_rewards_cost ON public.rewards(cost_tokens);
...
CREATE INDEX IF NOT EXISTS idx_user_rewards_active ON public.user_rewards(is_active);
...
CREATE INDEX IF NOT EXISTS idx_gamification_events_date ON public.gamification_events(created_at DESC);
...
-- =====================================================
-- 14. INICIALIZAR DADOS PARA USUÁRIOS EXISTENTES
-- =====================================================

-- Atualizar usuários existentes que não têm dados de gamificação
...
-- 2. Este script usa a tabela userprofile existente no schema public
-- 3. Verifique se todas as tabelas foram criadas corretamente
-- 4. Teste as políticas RLS com diferentes usuários
-- 5. Monitore a performance com os índices criados
-- 6. Ajuste as recompensas e missões conforme necessário
-- 7. Os dados de gamificação agora estão em public.userprofile
-- 8. Usuários existentes serão inicializados automaticamente