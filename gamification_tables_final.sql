-- Script SQL FINAL para criar todas as tabelas necessárias para o sistema de gamificação
-- Usando auth.users diretamente com colunas de gamificação adicionadas
...
-- =====================================================
-- 2. ADICIONAR COLUNAS DE GAMIFICAÇÃO EM auth.users
-- =====================================================

-- Adicionar colunas de gamificação na tabela auth.users existente
...
CREATE INDEX IF NOT EXISTS idx_auth_users_xp ON auth.users(xp DESC);
CREATE INDEX IF NOT EXISTS idx_auth_users_level ON auth.users(level DESC);
CREATE INDEX IF NOT EXISTS idx_auth_users_tokens ON auth.users(tokens DESC);
CREATE INDEX IF NOT EXISTS idx_auth_users_last_activity ON auth.users(last_activity DESC);
...
-- 6. Os dados de gamificação agora estão diretamente em auth.users
-- 7. Usuários existentes serão inicializados automaticamente